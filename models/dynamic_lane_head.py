...
class QueryBranchHead(nn.Module):
    """Per-query MLP branches that emit raw control tensors for the dynamic head."""
    ...
    def _make_branch(
        ...
        output_bias_init: Union[torch.Tensor, None] = None,
    ) -> nn.Sequential:
    ...
    def forward(self, query_features: torch.Tensor) -> Dict[str, torch.Tensor]:
        return {
            ...
            'pred_ranges': self.ranges(query_features),
...
class DynamicSpatialBranch(nn.Module):
    """Single-layer dynamic 1x1 conv branch applied independently per query."""

    def __init__(self, feature_channels: int, out_channels: int = 1, use_coords: bool = True):
        super().__init__()
    ...
    def _split_dynamic_params(self, dynamic_params: torch.Tensor) -> torch.Tensor:
...
class DynamicLaneHead(nn.Module):
    """
    Minimal CondLSTR-style dynamic lane head.

    Single-tensor contract:
        feature_map:    [B, C_feat, H, W]
        query_features: [B, Q, C_query]

    Sequence contract:
        feature_map:    list/tuple of [B, C_feat, H_i, W_i]
        query_features: list/tuple of [B, Q, C_query]

    Returns a dictionary with:
        pred_object_logits: [B, Q, 2]
        pred_class_logits:  [B, Q, num_classes]
        pred_ranges:        [B, Q, 2]
        pred_mask_params:   [B, Q, P_mask]
        pred_reg_params:    [B, Q, P_reg]
        pred_dense_reg:     [B, Q, H, W] if reg_out_channels == 1 else [B, Q, C_reg, H, W]
        pred_dense_mask:    [B, Q, H, W] if mask_out_channels == 1 else [B, Q, C_mask, H, W]

    When list/tuple inputs are provided, each dictionary value becomes a list of per-layer outputs.
    """
    ...
    def _forward_single(self, feature_map: torch.Tensor, query_features: torch.Tensor) -> Dict[str, torch.Tensor]:
        if feature_map.dim() != 4:
            raise ValueError(f'feature_map must be 4D [B, C_feat, H, W], got shape {tuple(feature_map.shape)}')
        if query_features.dim() != 3:
            raise ValueError(
                f'query_features must be 3D [B, Q, C_query], got shape {tuple(query_features.shape)}'
            )
        if query_features.size(-1) != self.query_channels:
            raise ValueError(
                f'query_features channel mismatch: expected {self.query_channels}, got {query_features.size(-1)}'
            )
        if feature_map.size(0) != query_features.size(0):
            raise ValueError('feature_map and query_features batch size must match')
    ...
    def _forward_sequence(
        self,
        feature_maps: Union[torch.Tensor, Sequence[torch.Tensor]],
        query_features: Union[torch.Tensor, Sequence[torch.Tensor]],
    ) -> Dict[str, List[torch.Tensor]]:
        if not isinstance(feature_maps, (list, tuple)) or not isinstance(query_features, (list, tuple)):
            raise ValueError('feature_map and query_features must both be tensors or both be lists/tuples')
