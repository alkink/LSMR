...
CFG="LSTR_CULANE_2k_condlstr_parity_dn"
...
# Remote cache dosyalari local absolute path ile kirlenebiliyor.
find cache -maxdepth 1 -name 'culane_*.pkl' -delete

# Yeni run ile eski diagnostics karismasin.
...
python -m py_compile \
  ...
  models/LSTR_CULANE_2k_condlstr_parity_dn.py \
