...
BASE_CFG="LSTR_CULANE_5k_condlstr_parity_stdc_dn_qobj"
SEED2="${1:-901}"
ITER="${2:-30000}"
SUFFIX="${3:-thr04}"
...
bash remote_run_parity_5k_stdc_dn_qobj_train.sh

echo "[2/6] seed317 test+eval"
bash remote_run_parity_5k_stdc_dn_qobj_test_eval.sh "${ITER}" "${SUFFIX}"
...
rm -f "results/${SEED2_CFG}/match_diag_train.jsonl"
