"""
DENEY 2 BÖLÜM 2 — Pipeline uyumluluk (BASE / B / E odağı).
"""
...
def run(cfg_name: str = "LSTR_CULANE_2k_mamba_dec_b", d_state: int = 16, d_conv: int = 4, seed: int = 0) -> Dict[str, bool]:
    return run_pipeline(
        cfg_name=cfg_name,
        d_state=d_state,
        d_conv=d_conv,
...
def main():
    ...
    print("RAPOR")
    print("=" * 96)
    print(f"DENEY2 BÖLÜM2 sonucu: {'✅' if ok else '🚨'}")
    if not ok:
        raise SystemExit(1)
